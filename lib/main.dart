...
class _TFLiteDemoState extends State<TFLiteDemo> {
  ...
  Future<void> loadModel() async {
    try {
      ...
      _interpreter = await Interpreter.fromAsset('assets/MH100k_float16.tflite');
      
      // Load labels
      String labelsData = await rootBundle.loadString('assets/MH100k_labels_out.txt');
  ...
  try {
    ...
    //List<double> inputData = List.generate(24841, (index) => Random().nextInt(2).toDouble()); //RAND
    List<double> inputData = List.generate(24841, (index) => 0.0); //ONLY 0s
